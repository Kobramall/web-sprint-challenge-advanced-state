...
const onChange = evt => {
  ...
  return (
    <form id="form" onSubmit={onSubmit}>
      ...
      <button onClick ={() =>props.resetForm()} id="submitNewQuizBtn" disabled={ props.newQuestion.trim().length < 2 || props.newTrueAnswer.trim().length < 2 || props.newFalseAnswer.trim().length < 2 ? true : false} >Submit new quiz</button>
    </form>
  )
}
